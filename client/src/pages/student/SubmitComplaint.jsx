...
const highlightMatch = (text, query) => {
  ...
  return text
    ...
    .map((part, i) =>
      regex.test(part)
        ? <mark key={i} className="sc-highlight">{part}</mark>
        : part
    );
};
...
const CategoryCard = React.memo(({ icon, title, description, selected, onClick }) => (
  ...
  >
    ...
    <span className="sc-cat-card__icon" aria-hidden="true">{icon}</span>
  ...
  return (
    <div className="sc-field">
      <div className="sc-field__wrap">
        ...
        <label className="sc-field__label" htmlFor={id}>{label}</label>
      </div>
      {maxLength && (
        <div className="sc-field__footer">
          <span className={ctCls}>{value.length} / {maxLength}</span>
        </div>
      )}
    </div>
  );
    ...
    >
      ...
      >
        ...
        <span className="sc-location__trigger-actions">
          ...
          <span className={`sc-location__chevron ${open ? 'sc-location__chevron--up' : ''}`}>
            <Icon.Chevron />
          </span>
        </span>
      </button>
              ...
              filtered.map((group) => (
                <div key={group.label} className="sc-location__group">
                  ...
                  {group.items.map((item) => (
                    ...
                    >
                      <span className="sc-location__option-icon"
                        aria-hidden="true">
                        {item.icon}
                      </span>
                      ...
                      {value === item.id && (
                        <span className="sc-location__option-check"
                          aria-hidden="true">
                          <Icon.Check size={12} />
                        </span>
                      )}
          ...
          {step === 1 && (
            <div className="sc-step" key="step-1">
              ...
              <div className="sc-nav sc-nav--end">
                ...
                >
                  Continue
                  <span className="sc-btn__icon"><Icon.ArrowRight /></span>
                </button>
              </div>
            </div>
          )}
